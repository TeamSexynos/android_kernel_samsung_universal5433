 ...
 *
 * EXYNOS5433 - EAGLE Core frequency scaling support
 *
...
static struct apll_freq exynos5433_apll_freq_CA15[] = {
	/*
	 * values:
	 * freq
	 * clock divider for EGL1, EGL2, ACLK_EGL, PCLK_EGL, ATCLK, PCLK_DBG_EGL, SCLK_CNTCLK, RESERVED
	 * clock divider for SCLK_EGL_PLL, SCLK_HPM_EGL, RESERVED
	 * PLL M, P, S
	 */
...
static int exynos5433_bus_table_CA15[CPUFREQ_LEVEL_END_CA15] = {
	825000,		/* 2.5 GHz */
	825000,		/* 2.4 GHz */
	825000,		/* 2.3 GHz */
	825000,		/* 2.2 GHz */
...
static void exynos5433_set_clkdiv_CA15(unsigned int div_index)
{
	...
	/* Change Divider - EGL0 */
	tmp = exynos5433_apll_freq_CA15[div_index].clk_div_cpu0;
	...
	/* Change Divider - EGL1 */
	tmp1 = exynos5433_apll_freq_CA15[div_index].clk_div_cpu1;
...
static void exynos5433_set_egl_pll_CA15(unsigned int new_index, unsigned int old_index)
{
	...
	pdiv = ((exynos5433_apll_freq_CA15[new_index].mps &
	...
	tmp |= exynos5433_apll_freq_CA15[new_index].mps;
...
static bool exynos5433_pms_change_CA15(unsigned int old_index,
				      unsigned int new_index)
{
	unsigned int old_pm = (exynos5433_apll_freq_CA15[old_index].mps >>
				EXYNOS5430_PLL_PDIV_SHIFT);
	unsigned int new_pm = (exynos5433_apll_freq_CA15[new_index].mps >>
				EXYNOS5430_PLL_PDIV_SHIFT);

	return (old_pm == new_pm) ? 0 : 1;
}

static void exynos5433_set_frequency_CA15(unsigned int old_index,
					 unsigned int new_index)
{
	unsigned int tmp;
	...
	sec_debug_aux_log(SEC_DEBUG_AUXLOG_CPU_BUS_CLOCK_CHANGE,
		"[A15] %7d <= %7d",
		exynos5433_freq_table_CA15[new_index].frequency, exynos5433_freq_table_CA15[old_index].frequency);

	if (old_index > new_index) {
		if (!exynos5433_pms_change_CA15(old_index, new_index)) {
			/* 1. Change the system clock divider values */
			exynos5433_set_clkdiv_CA15(new_index);
			/* 2. Change just s value in egl_pll m,p,s value */
			tmp = __raw_readl(EXYNOS5430_EGL_PLL_CON0);
			tmp &= ~EXYNOS5430_PLL_SDIV_MASK;
			tmp |= (exynos5433_apll_freq_CA15[new_index].mps & EXYNOS5430_PLL_SDIV_MASK);
			__raw_writel(tmp, EXYNOS5430_EGL_PLL_CON0);
		} else {
			...
			exynos5433_set_egl_pll_CA15(new_index, old_index);
		}
	...
	clk_set_rate(fout_egl_pll, exynos5433_freq_table_CA15[new_index].frequency * 1000);
	pr_debug("[%s] Eagle NewFreq: %d OldFreq: %d\n", __func__, exynos5433_freq_table_CA15[new_index].frequency,
										exynos5433_freq_table_CA15[old_index].frequency);
}
...
static void __init set_volt_table_CA15(void)
{
	...
	for (i = 0; i < CPUFREQ_LEVEL_END_CA15; i++) {
		...
		if (!asv_volt)
			exynos5433_volt_table_CA15[i] = asv_voltage_5433_CA15[i];
		else
			exynos5433_volt_table_CA15[i] = asv_volt;

		pr_info("CPUFREQ of CA15  L%d : %d uV\n", i,
				exynos5433_volt_table_CA15[i]);

		exynos5433_abb_table_CA15[i] =
			get_match_abb(ID_ARM, exynos5433_freq_table_CA15[i].frequency);
	...
	pr_info("CPUFREQ of CA15 min_freq : L%d %u khz\n", min_support_idx_CA15,
		exynos5433_freq_table_CA15[min_support_idx_CA15].frequency);
}

static bool exynos5433_is_alive_CA15(void)
{
	unsigned int tmp;
...
#ifdef CONFIG_SEC_PM
static void set_boot_cpu_qos_freq(struct exynos_dvfs_info *info, int lv_idx)
{
...
#endif
	info->boot_cpu_min_qos = exynos5433_freq_table_CA15[lv_idx].frequency;
	info->boot_cpu_max_qos = exynos5433_freq_table_CA15[lv_idx].frequency;
}
#endif
...
int __init exynos5_cpufreq_CA15_init(struct exynos_dvfs_info *info)
{
	...
	mout_egl = clk_get(NULL, "mout_egl");
	if (IS_ERR(mout_egl)) {
	...
	fout_egl_pll = clk_get(NULL, "fout_egl_pll");
	...
	info->volt_table = exynos5433_volt_table_CA15;
	info->abb_table = exynos5433_abb_table_CA15;
	info->freq_table = exynos5433_freq_table_CA15;
