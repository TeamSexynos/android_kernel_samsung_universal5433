...
void dw_pcie_prog_viewport_cfg0(struct pcie_port *pp, u32 busdev)
{
	...
	dw_pcie_readl_rc(pp, dbi_base + PCIE_ATU_VIEWPORT, &val);
...
void dw_pcie_setup_rc(struct pcie_port *pp)
{
	...
	val &= 0xfffffc00;
	dw_pcie_writel_rc(pp, val | 0x7 << 15 | 0x1 <<4 | 0x2 << 0, dbi_base + PCIE_LINK_CAP);

	dw_pcie_writel_rc(pp, 0x18, dbi_base + PCIE_AUX_CLK_FREQ_OFF);
	dw_pcie_writel_rc(pp, 0x0, dbi_base + PCIE_CON_STATUS);
	...
	dw_pcie_readl_rc(pp, dbi_base + PCIE_LINK_CTRSTS2, &val);
	val &= ~0xf;
	val |= 0x1;
	dw_pcie_writel_rc(pp, val, dbi_base + PCIE_LINK_CTRSTS2);
}
